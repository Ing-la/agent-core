...
### `contrib/mcp_bridge.py` — MCP 协议适配器

将 MCP (Model Context Protocol) 服务暴露的工具转为 agent-core Tool 实例。

- 通过 stdio 连接 MCP 服务器，调用 `tools/list` 获取工具列表
- 每个 MCP 工具包装为 `_MCPTool(Tool)`，Engine 无感知
- 只 `from agent_core.contrib import MCPBridge` 时才需要 `pip install mcp`
...
contrib/
  ...
  └── mcp_bridge.py      → tool.py (Tool), mcp (可选依赖)
```
